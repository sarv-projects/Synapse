...
def schedule_hourly_reset(app):
    ...
    if not _reset_task_scheduled:
        ...
        async def cycle_reset():
            """Run initial reset immediately, then every hour.

            Previously the first reset only happened after a full hour of uptime.
            """
            try:
                await hourly_reset_task()
            except Exception as e:
                logger.error(f"Initial Groq TPM reset failed: {e}", exc_info=True)
