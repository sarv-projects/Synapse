...
async def output_node(state: ReasoningState) -> ReasoningState:
    ...
    markdown = state.get("synthesis_markdown", "")
    if not markdown:
        markdown = f"## Summary\nNo synthesis was produced for query: {state['query']}\n"
        markdown += f"\n## Knowledge Gaps\nUnable to process this query. {state.get('error', '')}"
    ...
    state["knowledge_gaps"] = state.get("critic_result", {}).get("missing_sub_questions", [])
    state["final_markdown"] = markdown
    state["produced_by"] = "markdown"

    # ── RAGAS evaluation ────────────────────────────────────────────────
    total_tokens = sum(state.get("total_tokens_used", {}).values()) if state.get("total_tokens_used") else 0
    contexts = [
        r.get("content") or r.get("snippet") or str(r)
        for r in state.get("retrieval_context", [])[:20]
    ]
    try:
    ...
    logger.info(
        f"Output: produced_by={state.get('produced_by', '')}, "
        f"markdown={len(state.get('final_markdown', ''))} chars, sources={len(sources)}"
    )
