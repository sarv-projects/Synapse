"""Node 3: Retrieval — hybrid retrieval over Neo4j (BM25 + KG) and pgvector."""
import logging
...
async def retrieval_node(state: ReasoningState) -> ReasoningState:
    ...
    for sq in state["sub_questions"]:
        ...
        try:
            ...
            cache_results = await store.search_similar_async(qv, limit=5, score_threshold=0.85)
    ...
    confidence_threshold = get_settings().retrieval_confidence_threshold
    ...
    if result_count == 0:
        ...
        state["retrieval_confidence"] = min(confidence_threshold, (result_count / 10) * 0.5)
