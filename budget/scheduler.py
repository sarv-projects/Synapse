...
Uses an async context-manager pattern so release() is ALWAYS called,
even when the guarded task raises an exception. This prevents semaphore
leaks that would starve the scheduler over time.
"""
import asyncio
import logging
import random
...
class LeakyBucketScheduler:
    ...
    async def acquire(self, model_id: str, max_retries: int = 10) -> None:
        """Acquire semaphore with bounded retries so the loop can never spin forever."""
        ...
        while retries < max_retries:
            try:
                ...
                if retries >= max_retries:
                    ...
                    raise RuntimeError(
                        f"Failed to acquire semaphore for {model_id} after {max_retries} attempts"
                    )
                delay = min(delay + 1 + random.uniform(0, 1), max_delay)
                logger.debug(f"LeakyBucket: {model_id} waiting {delay:.1f}s ({retries}/{max_retries})")
                await asyncio.sleep(delay)

    async def __aenter__(self) -> None:
        """No-op: use scoped() for the context-manager pattern."""
        return

    async def __aexit__(self, *args) -> None:
        return None

    async def scoped(self, model_id: str, max_retries: int = 10) -> AsyncIterator[None]:
        """Context manager: acquire on enter, ALWAYS release on exit (even on exception)."""
        await self.acquire(model_id, max_retries)
        try:
            yield
        finally:
